# Product schema — CareFlow (PostgreSQL 16 + pgvector)

Canonical physical model for the Kenya pretriage PWA. Implements [product-spec.md](product-spec.md), [user-journeys.md](user-journeys.md) J1–J9, [kenya-pretriage.md](kenya-pretriage.md), [D-001](../research/decision-log.md), and [docs/research/postgresql-primary-store.md](../docs/research/postgresql-primary-store.md).

**Not a diagnosis store. Not SHA/AfyaKE. Not a medical device.** Firebase remains auth only (`firebase_uid`). Notes image **bytes** are out of Postgres (URLs + OCR text only).

P1 turns this file into the first Alembic revision. Do not invent a second facility schema or a diagnosis engine.

**Whiteboard photos** (product-intent, not a second product): Photo A is the care-seeker profile + book funnel (preferred hospital / prepaid / department / slot). Photo B is post-book **Appointment → Confirmation** XOR **Instant → queue position**. Where a photo disagrees with the locked spec, MVP columns satisfy J1–J9 now; nullable FKs and subtype tables are the extension points so the whiteboard can land without a rewrite.

---

## 1. Entity list

**Booking is a supertype** (`bookings`) with two exclusive subtypes: **instant / walk-in** (`booking_instant`) and **appointment** (`booking_appointments`). Shared facts live on the parent: facility, patient, discovery path, status, locale, rule snapshot (`keph_min_applied`, `red_flag_applied`), optional department, optional free-text (never the sole triage input). Instant means “book this facility now after KEPH ranking” (spec J1/J2) and is the **only kind MVP writes**. Appointment means a future slot plus `confirmed_at` (whiteboard Photo B); MVP does not insert that subtype. A deferred constraint trigger requires exactly one subtype row matching `booking_kind`. Prepaid hospital is **not** a subtype and **not** a ledger — it is only `booking_channel = prepaid_partner` (money deferred).

Other entities: **`facilities`** (KMHFR cache + seed; Kenya, KEPH 2–6, NOT NULL coordinates, desk `wait_count` for ranking); **`departments`** (facility-scoped catalog; bookings may reference, MVP leaves `department_id` NULL); **`symptoms`** + **`symptom_synonyms`** (catalog + per-language phrase + pgvector embedding); **`users`** (thin auth: Firebase, role, staff `facility_id`, UI locale, `+254` phone); **`patient_profiles`** (1:1 PII: legal name, DOB, nationality, typed ID); **`booking_symptoms`** (N:M; arrays forbidden); **`booking_facility_snapshots`** (frozen name/KEPH/geo/wait at book time); **`notes`** + **`note_images`**; **`notify_jobs`**; **`user_preferred_facilities`** (whiteboard preferred-hospital path; empty in MVP).
...
|--------------|-------------|--------|
| `users` — `patient_profiles` | 1:1 | PK = `user_id`. Staff have no profile row. |
| `users` (patient) — `bookings` | 1:N | `patient_user_id` |
| `users` (staff) — `facilities` | N:1 | `users.facility_id` NOT NULL iff `role = hospital_staff` |
| `facilities` — `departments` | 1:N | Unique `(facility_id, slug)` |
| `facilities` — `bookings` | 1:N | Staff isolation is this FK + RLS |
| `bookings` — `booking_instant` / `booking_appointments` | 1:0..1 exclusive | Exactly one, matching `booking_kind` |
| `bookings` — `symptoms` | N:M via `booking_symptoms` | ≥1 symptom per booking (deferred trigger) |
| `bookings` — `booking_facility_snapshots` | 1:1 | Inserted in the book transaction |
| `bookings` — `notes` | 1:N | No patient SELECT (RLS). No DELETE. |
| `notes` — `note_images` | 1:N | URLs only; per-image OCR optional |
| `bookings` — `notify_jobs` | 1:N | SMS / call / Pawa-play; JSONB vendor payload only |
| `users` — `user_preferred_facilities` | 1:N | Whiteboard path 1; unused in MVP |
| `departments` — `bookings` | 1:N optional | Composite FK so department cannot be another facility’s |

Queue **position** is **not** an entity: it is derived from instant bookings still `booked` at that facility (view `v_instant_queue_positions`). Facility **`wait_count`** is a different fact (ranking input, desk-reported, bumped by instant book / terminal status).

```mermaid
erDiagram
  users ||--o| patient_profiles : "1:1 PII"
  users ||--o{ bookings : "patient 1:N"
  facilities ||--o{ users : "staff N:1"
  facilities ||--o{ departments : "1:N"
  facilities ||--o{ bookings : "1:N"
  bookings ||--o| booking_instant : "kind=instant"
  bookings ||--o| booking_appointments : "kind=appointment"
  bookings ||--|{ booking_symptoms : "N:M"
  symptoms ||--|{ booking_symptoms : "N:M"
  symptoms ||--o{ symptom_synonyms : "1:N"
  bookings ||--|| booking_facility_snapshots : "1:1 freeze"
  bookings ||--o{ notes : "1:N staff"
  notes ||--o{ note_images : "1:N URLs"
  bookings ||--o{ notify_jobs : "1:N"
  departments ||--o{ bookings : "optional"
```

---

## 3. Normalization commentary

The model is **3NF**: every non-key attribute depends on the key, not on another non-key. Symptom catalog is not copied onto bookings except through `booking_symptoms`. Facility name/KEPH/coordinates are **not** stored on `bookings`; they are frozen in `booking_facility_snapshots` so J1 confirmation (name, KEPH, map, wait) and later audit survive KMHFR recodes — that snapshot is a **named historical invariant**, not a live cache. ID numbers are typed (`id_document_type`) plus HMAC + last-4, not a free-text blob. `symptom_ids[]` is forbidden; `image_urls[]` is replaced by `note_images`.

**Deliberate denormalization (one):** `facilities.wait_count` is a desk-maintained integer used for ranking ~17k rows (`keph_level >= keph_min`, then `wait_count ASC`, then distance). It is **not** `COUNT(*)` of open bookings. Walk-ins outside the PWA exist (J4). Instant book / arrived / no-show / cancel adjust it in the same transaction as the booking row so ranking cannot diverge mid-write. Desk `PATCH` may still set it independently; **drift is allowed**; **negative is not**.

**Generated / derived (not stored):** instant **queue position** (Photo B) = `1 + count` of other `booking_instant` rows at the same facility with `status = 'booked'` and earlier `(created_at, id)`. Do not store position; it would race and would be confused with `wait_count`. Nairobi “today” for the hospital queue is a **display/filter** on `timestamptz` (`AT TIME ZONE 'Africa/Nairobi'`), not a stored date column.

**Not denormalized:** `keph_min_applied` / `red_flag_applied` on `bookings` are the **rule output at book time** (catalog rows can change later). They are functionally dependent on the booking event, not on current `symptoms.keph_min`.

---

## 4. PostgreSQL DDL

Postgres **16**. Extensions: `vector` (required, D-001); `cube` + `earthdistance` for ranking (not PostGIS).

**Geo pick:** `cube` / `earthdistance` (`ll_to_earth`, `earth_distance` in metres). ~17k Kenya points, bounding-box CHECKs, and a GiST index are enough for `ORDER BY wait_count, distance`. PostGIS would force a different compose image than `pgvector/pgvector:pg16` (contrib `earthdistance` is already there). Revisit PostGIS only if we need polygons, KMHFR catchment, or Esri QA joins — Esri is not SoT.

**Vector:** `vector(384)` = named constant **`SYMPTOM_EMBEDDING_DIM`**. Model constant **`SYMPTOM_EMBEDDING_MODEL = 'intfloat/multilingual-e5-small'`** (multilingual; covers synonym langs better than an English-only MiniLM). Store **L2-normalized** vectors; search with cosine (`<=>` / `vector_cosine_ops`). **HNSW** not IVFFlat: catalog is ~100–200 symptoms × langs × phrases (thousands of rows, not millions); IVFFlat needs a train pass and goes stale when synonyms are added. Index is for headroom — at this size a seq scan is acceptable.

**Model change:** do not mix dimensions in one column. New Alembic revision: drop HNSW → `ALTER … TYPE vector(new_dim)` → backfill job writes new embeddings and `embedding_model` → recreate HNSW. App config must match `embedding_model` or refuse `/symptoms/map`.

**PKs:** `BIGINT GENERATED ALWAYS AS IDENTITY` (ADR). No UUID (no client-generated ids, no multi-primary replication).

**Time:** `timestamptz` only. `Africa/Nairobi` is display / “today” filtering.

**Deletes:** no `DELETE` on notes or bookings. Terminal `booking_status` + timestamps. Notes have no `deleted_at`.

**JSONB:** only `notify_jobs.vendor_payload` (vendor request/response). Comment on the column.
...
### Recommend query shapes (not stored procedures)

Routine (J1): `operational AND keph_level >= :keph_min ORDER BY wait_count ASC, earth_distance(ll_to_earth(lat, lng), ll_to_earth(:lat, :lng)) ASC`.

Red flag (J2): `operational AND keph_level >= 4 ORDER BY earth_distance(...) ASC` — **do not** order by `wait_count`. Booking is still allowed.

Hospital queue (J5): `bookings` for `facility_id = app.facility_id` and Nairobi calendar day on `created_at` (instant) or `slot_start` (appointment, later), `status = 'booked'`. Display `given_name` / `family_name` and `right(phone_e164, 4)` — never `id_number_hmac` / ciphertext.

Synonym map: `ORDER BY embedding <=> :q LIMIT k` with optional `lang IN (:utterance_lang, 'en', 'sw')`. Set `hnsw.ef_search = 40` in the session if using the index.

### Concurrency: book + wait_count (lock order)

**Always lock the `facilities` row (`SELECT … FOR UPDATE`) before inserting or updating the booking** when both are in the same transaction. Desk `PATCH` also `FOR UPDATE`s the facility row. That serialization is what keeps `wait_count` correct under concurrent desk edits and concurrent books at ~17k facilities (only the one facility row is contended).

**Instant create** (MVP; one ACID transaction):

```sql
BEGIN;
SELECT id, wait_count FROM facilities
 WHERE id = :facility_id AND operational
 FOR UPDATE;

INSERT INTO bookings (… booking_kind = 'instant', status = 'booked', …)
  VALUES (…) RETURNING id;
INSERT INTO booking_instant (booking_id) VALUES (:id);
INSERT INTO booking_symptoms (…) VALUES (…);  -- ≥1 row
INSERT INTO booking_facility_snapshots (… wait_count_at_book = facilities.wait_count …);

UPDATE facilities
   SET wait_count = wait_count + 1
 WHERE id = :facility_id;

COMMIT;  -- deferred subtype + symptom triggers fire here
```

**Arrived / no_show / cancel** (decrement once; never negative). Lock the **facility first** (same order as create) to avoid deadlocks with concurrent books. Decrement only if this statement actually left `booked` (idempotent J5 double-submit):
...
SELECT id, facility_id, booking_kind, status
  FROM bookings
 WHERE id = :booking_id
 FOR UPDATE;
...
WITH moved AS (
  UPDATE bookings
     SET status = 'arrived',          -- or no_show / cancelled
         arrived_at = now(),          -- or no_show_at / cancelled_at
         updated_at = now()
   WHERE id = :booking_id
     AND status = 'booked'
...
Appointment terminal updates omit `booking_kind = 'instant'` in `moved` (or use a separate `UPDATE bookings` without the `facilities` write). If `moved` is empty, `wait_count` is unchanged.

Use `wait_count > 0` (and the table CHECK) so a desk PATCH to `0` cannot violate the constraint.

**Desk PATCH (J4):** `SELECT … FOR UPDATE` on that facility only; `UPDATE facilities SET wait_count = :n WHERE id = :staff.facility_id`. Never read/write another facility’s row.

**Appointments** do **not** change `wait_count` until a later check-in rule exists. MVP does not write `booking_appointments`.

---

## 5. Invariants (testable)

1. **`facilities.wait_count >= 0`** always (CHECK). Instant `booked` create increments by 1 in the same transaction as the booking + `booking_instant` + ≥1 `booking_symptoms` + snapshot. Transition `booked` → `arrived` | `no_show` | `cancelled` decrements by 1 iff `booking_kind = 'instant'` and the status update matches exactly one row. Second terminal update does not decrement again.

2. **`wait_count` ≠ queue position.** Ranking reads `facilities.wait_count`. Photo B position is `v_instant_queue_positions.queue_position` (or the equivalent `COUNT(*)` query). No column stores position.

3. **Staff facility isolation.** `hospital_staff.facility_id` is NOT NULL. Queue, wait PATCH, arrived/no-show, and notes only succeed when `bookings.facility_id = users.facility_id`. RLS FORCE on `bookings` / `notes` / `note_images`. A staff JWT for facility A returns zero rows for facility B.

4. **Patients cannot read notes.** No RLS policy grants `role=patient` on `notes` or `note_images`. Trigger rejects note authors who are not `hospital_staff` of the booking’s facility. API `/hospital/notes` is staff-only (J6).

5. **Red-flag bookings are allowed.** No CHECK forbids `red_flag_applied = TRUE`. J2 still inserts `booking_kind = 'instant'`. Recommend for those sessions ignores wait and requires `keph_level >= 4`.

6. **Triage input is catalog, not a blob.** Every booking has ≥1 `booking_symptoms` row (deferred trigger). `patient_free_text` is optional and unconstrained as diagnosis.

7. **Exclusive booking arc.** `booking_kind = instant` ↔ one `booking_instant` and zero `booking_appointments`; the inverse for appointment (deferred trigger).

8. **Auth vs PII.** `users` has no national ID, DOB, or legal name. Those live only on `patient_profiles`. ID HMAC/ciphertext/last-4 are never written to `notify_jobs.vendor_payload` or application logs.

9. **Kenya-only phones and facilities.** `phone_e164 ~ ^+254[17][0-9]{8}$`. Facility lat/lng NOT NULL and inside the Kenya bbox. `keph_level` ∈ [2, 6]. `source ∈ {seed, kmhfr}` — no Esri product rows.

10. **No silent clinical deletes.** No `ON DELETE CASCADE` from bookings to notes. Application does not `DELETE FROM notes`. Booking end-of-life is `cancelled` / `arrived` / `no_show`.

11. **Subtype wait coupling.** Instant + `booked` is the only state that has applied `wait_count + 1` and not yet `-1`. Appointments never bump `wait_count` in this schema version.

12. **Snapshot freeze.** After insert, `booking_facility_snapshots` is immutable (no app UPDATEs). Live `facilities.name` / `keph_level` may change on KMHFR sync.

---

## 6. MVP vs later

| Column / table | Status | Reason |
|----------------|--------|--------|
| `facilities.*` (except Esri) | `mvp_required` | J1–J2, J4, J7 ranking + seed |
| `users.*` | `mvp_required` | Auth, staff scope, SMS target |
| `patient_profiles.given_name`, `family_name` | `whiteboard_ready_nullable` | J5 wants a name; Firebase-only users can book; fallback = phone last-4 |
| `patient_profiles.date_of_birth`, `nationality_iso2`, home lat/lng/label | `whiteboard_ready_nullable` | Photo A #1; not in spec User |
| `patient_profiles.id_document_type`, `id_number_last4`, `id_number_hmac`, `id_hmac_key_version` | `whiteboard_ready_nullable` | Typed ID; HMAC uniqueness; J5 does not display full ID |
| `patient_profiles.id_number_ciphertext` | `deferred` | Full-ID envelope when a hospital workflow needs it |
| `symptoms.*`, `symptom_synonyms.*` | `mvp_required` | Map + rules; pgvector |
| `bookings` instant + `ranked_recommend` + `department_id` NULL | `mvp_required` | Spec book-now |
| `booking_instant` | `mvp_required` | MVP subtype |
| `booking_symptoms` | `mvp_required` | No arrays |
| `booking_facility_snapshots` | `mvp_required` | J1 confirmation after KMHFR updates |
| `bookings.patient_free_text` | `mvp_required` nullable | J1 optional extra text |
| `bookings.keph_min_applied`, `red_flag_applied` | `mvp_required` | Rule snapshot |
| `bookings.department_id` | `whiteboard_ready_nullable` | Photo A department; MVP null |
| `departments` | `whiteboard_ready_nullable` | Catalog can be empty |
| `booking_channel = preferred_search` | `whiteboard_ready_nullable` | Path 1; MVP writes `ranked_recommend` |
| `user_preferred_facilities` | `whiteboard_ready_nullable` | Preferred hospital list |
| `booking_kind = appointment`, `booking_appointments` | `whiteboard_ready_nullable` | Photo B; MVP does not insert |
| `booking_channel = prepaid_partner` | `deferred` | SHA-adjacent; enum exists; no ledger |
| Payments / SHA claims tables | `deferred` | Out of scope |
| `notes.*`, `note_images.*` | `mvp_required` | J6; URLs not bytes |
| `notify_jobs.*` | `mvp_required` | J1/J3/J9; `demo_log` in CI |
| `v_instant_queue_positions` | `mvp_required` | Photo B instant UX; may hide in UI until polish |
| `v_queue_patient_display` | `mvp_required` | J5 name + phone last-4; join via staff facility bookings |
| Appointment slot uniqueness / department schedule | `deferred` | No double-book constraint yet |
| PostGIS, websockets, inbound IVR, Pawa-native PSTN | `deferred` | Out of scope |
| Separate admin role table | `deferred` | Desk = clinician login |
| `wait_count` column on bookings | **forbidden** | Would collapse ranking wait with queue position |

---

## 7. Open questions

None that block this DDL. Locked choices above: earthdistance (not PostGIS), `SYMPTOM_EMBEDDING_DIM = 384` / e5-small, HMAC of ID in-app (key in Phantom), appointments do not touch `wait_count`. If P2 picks a different embedding model, that is one Alembic revision (section 4 model-change path), not a schema rewrite.
