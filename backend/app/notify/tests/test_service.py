...
def test_booking_confirm_demo_logged(sample_booking_id, monkeypatch):
...
def test_no_show_sms(sample_booking_id, monkeypatch):
    monkeypatch.setenv("DEMO_NOTIFY", "1")
    from app.core.config import get_settings
...
def test_reminder_creates_sms_and_call_jobs(sample_booking_id, monkeypatch):
    monkeypatch.setenv("DEMO_NOTIFY", "1")
    from app.core.config import get_settings
